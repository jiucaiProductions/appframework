...
public class MailSender {
	...
	private MailSender(final boolean isMultipart) throws Exception {
		...
		mimeMessageHelper = new MimeMessageHelper(
				javaMailSender.createMimeMessage(), isMultipart, "GBK");
	}
	...
	public void sendMailAsText(final String fromTitle, final String mailFrom,
			final String[] toTitles, final String[] mailTos,
			final String[] mailCCs, final String subject, final String strText)
			throws Exception {
		sendMail(fromTitle, mailFrom, toTitles, mailTos, mailCCs, subject,
				strText, false, null, true);
	}
	 ...
	 */
	public void sendMailAsTextHasAtts(final String fromTitle,
			final String mailFrom, final String[] toTitles,
			final String[] mailTos, final String[] mailCCs,
			final String subject, final String strText,
			final Hashtable<String,File> fileTable) throws Exception {
		sendMail(fromTitle, mailFrom, toTitles, mailTos, mailCCs, subject,
				strText, false, fileTable, true);
	}
	 ...
	 *            附件表key为文件名value为File对象
	 */
	public void sendMailAsHtmlHasAtts(final String fromTitle,
			final String mailFrom, final String[] toTitles,
			final String[] mailTos, final String[] mailCCs,
			final String subject, final String strText,
			final Hashtable<String,File> fileTable) throws Exception {
		sendMail(fromTitle, mailFrom, toTitles, mailTos, mailCCs, subject,
				strText, true, fileTable, true);
	}
	 ...
	 *            附件表key为文件名value为File对象
	 */
	private void sendMail(final String fromTitle, final String mailFrom,
			final String[] toTitles, final String[] mailTos,
			final String[] mails, final String subject, final String strText,
			final boolean html, final Hashtable<String,File>  fileTable, final boolean isCC)
			throws Exception {
		if (null != mailTos) {
			if (null == toTitles) {
		...
		if (null != fileTable) {
			...
			while (e.hasMoreElements()) {
				...
				final File value = (File) fileTable.get(key);
